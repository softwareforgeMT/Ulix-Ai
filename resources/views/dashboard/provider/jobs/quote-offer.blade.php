          ...
          @forelse($offers as $offer)
            <div class="bg-white rounded-xl border border-blue-300 shadow p-4 mb-4">
              ...
              <div class="flex gap-3">
                @if(auth()->check() && $mission && $mission->requester_id == auth()->id())
                  <button type="button" onclick="chooseProvider({{$offer->provider->id}}, {{$mission->id}})" class="text-sm bg-yellow-400 text-black px-4 py-1.5 rounded-full font-semibold hover:bg-yellow-500 transition">
...
<script>
  ...
  //Choose Provider
  function chooseProvider(provider, mission_id) {
    const modal = document.getElementById('confirmProviderModal');
      ...
      list.innerHTML += `
        <div class="space-y-3">
          <div class="flex gap-3 items-start">
            <img src="${msg.user.profile_photo ? '{{ asset('') }}' + msg.user.profile_photo : 'https://i.pravatar.cc/100?u=' + encodeURIComponent(msg.user.name)}" class="w-10 h-10 rounded-full" />
            <div class="text-sm">
